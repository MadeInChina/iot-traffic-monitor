...
//        kafkaParams.put("bootstrap.servers", "localhost:9092,anotherhost:90929092");
        ...
        final JavaInputDStream<ConsumerRecord<String, IoTData>> directKafkaStream =
                KafkaUtils.createDirectStream(
                        ...
                        ConsumerStrategies.<String, IoTData>Subscribe(topics, kafkaParams)
                );

        logger.info("Starting Stream Processing");

        //We need non filtered stream for poi traffic data calculation
        JavaDStream<IoTData> nonFilteredIotDataStream = directKafkaStream.map(cr -> cr.value());
